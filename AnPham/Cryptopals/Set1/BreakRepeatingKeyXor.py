...
def getValueHammingtonBlocks(s, blocksize):
  s0 = s[:blocksize]
  s1 = s[blocksize:blocksize*2]
  #s2 = s[blocksize*2:blocksize*3]

  d0 = hammingDistance(s0, s1) / blocksize
  #d1 = hammingDistance(s0, s2) / blocksize
  #d2 = hammingDistance(s1, s2) / blocksize
...
def decryptRepeatedKey(key, cipher, english = False, threshold = 2):
  global printable, english_words
  result = ''
...
G = []
res = []
expectedKey = 'Tbq?hi?rhq??9?Ag?i??sh??noise'
cnt = 3

def dequy(index):
  global t, key, cipher, printable, G, cnt, expectedKey
  if cnt < 0: return
  if index == len(key):
